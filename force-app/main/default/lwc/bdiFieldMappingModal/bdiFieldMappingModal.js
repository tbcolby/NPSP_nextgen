...
export default class bdiFieldMappingModal extends LightningElement {
    ...
    get isTargetFieldDisabled() {
        if (
            (this.fieldMapping.Source_Field_API_Name || this.fieldMapping.Source_Field_Label) &&
            this.targetFieldLabelOptions &&
            this.targetFieldLabelOptions.length > 0
        ) {
            return false;
        }
        return true;
    ...
    collectMappedTargetMappings(fieldMappings) {
        this.mappedTargetFieldApiNames = fieldMappings.map((fieldMapping) => {
    ...
    getSourceFieldOptions(fieldInfos) {
        try {
            ...
            for (let i = 0; i < fieldInfos.length; i++) {
                ...
                if (
                    !this.mappedDiFieldDescribes.includes(fieldInfos[i].value.toLowerCase()) ||
    ...
    handleSave() {
        try {
            ...
            if (missingField.length === 0) {
                ...
                showToast("Error", `Missing the following field ${missingField}`, "error", "dismissable");
            }
        } catch (error) {
            this.handleError(error);
        }
    ...
    handleFieldValidations() {
        ...
        if (
            (!this.fieldMapping.Target_Field_API_Name && !this.isTargetFieldDisabled) ||
