...
import getRelationshipFieldOptions from "@salesforce/apex/BDI_ManageAdvancedMappingCtrl.getRelationshipFieldOptions";
import getMappedDISourceFields from "@salesforce/apex/BDI_ManageAdvancedMappingCtrl.getMappedDISourceFields";
import getDataImportObjectName from "@salesforce/apex/BDI_ManageAdvancedMappingCtrl.getDataImportObjectName";
...
export default class bdiObjectMappingModal extends LightningElement {
     ...
     */
    validateFields() {
        let result = [...this.template.querySelectorAll("lightning-combobox,lightning-input")].reduce(
            (validSoFar, inputCmp) => {
                ...
                if (inputCmp.name === "masterLabel" && this.row.MasterLabel && this.objectMappings) {
                    ...
                    if (this.row.MasterLabel.length > 40) {
                        ...
                        for (let i = 0; i < this.objectMappings.length; i++) {
                            ...
                            if (
                                tempObjMapping.MasterLabel === this.row.MasterLabel &&
        ...
        if (!result) {
            this.showToast("Error", this.customLabels.bdiOMUIErrorInvalidValues, "error", "dismissable", null);
        }
    ...
    getRelationshipFieldOptions() {
        ...
        try {
            if (this.row.Object_API_Name && this.row.Relationship_To_Predecessor && this.row.Predecessor) {
                ...
                if (objectName) {
                    this.isLoading = true;
                    getRelationshipFieldOptions({ objectName: objectName, lookupToObjectName: lookupToObjectName })
                        .then((data) => {
    ...
    getDataImportFieldDescribes() {
        getObjectFieldDescribes({ objectName: this._dataImportApiName, includeReferenceToObjectList: true })
            .then((data) => {
            ...
            for (let i = 0; i < this.dataImportFieldData.length; i++) {
                ...
                if (!fieldsToExclude.has(fieldData.value.toLowerCase()) && !fieldData.isFormula) {
                    ...
                    if (fieldData.value !== this.row.Imported_Record_Status_Field_Name) {
                        if (
                            fieldData.displayType === "REFERENCE" &&
                    ...
                    if (
                        fieldData.value !== this.row.Imported_Record_Field_Name &&
            ...
            if (importedRecordField) {
                if (this.diImportRecordFieldOptions.length === 0) {
                    importedRecordField.setCustomValidity(
                        this.customLabels.bdiOMUIErrorNoUnmappedFieldsPt1 +
                            " " +
    ...
    getAlreadyMappedDIFields() {
        ...
        if (this.objectMappings && this.dataImportFieldMappingSourceNames && this.namespaceWrapper) {
            ...
            for (let i = 0; i < this.objectMappings.length; i++) {
                ...
                if (
                    tempObjMapping.Imported_Record_Field_Name &&
                    tempObjMapping.Imported_Record_Field_Name !== this.row.Imported_Record_Field_Name
                ) {
                    this.alreadyMappedDIFieldsMap.set(
                        tempObjMapping.Imported_Record_Field_Name.toLowerCase(),
                        tempObjMapping.MasterLabel
                    );
                }

                if (
                    tempObjMapping.Imported_Record_Status_Field_Name &&
                    tempObjMapping.Imported_Record_Status_Field_Name !== this.row.Imported_Record_Status_Field_Name
                ) {
                    this.alreadyMappedDIFieldsMap.set(
                        tempObjMapping.Imported_Record_Status_Field_Name.toLowerCase(),
                        tempObjMapping.MasterLabel
                    );
                }
            }
