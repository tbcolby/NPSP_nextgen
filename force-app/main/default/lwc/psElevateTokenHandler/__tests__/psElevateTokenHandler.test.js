...
describe("c-ps-Elevate-Token-Handler", () => {
    ...
    it("should discard an invalid message", async () => {
        ...
        await flushPromises();
        const isMessageHandled = psElevateTokenHandler.shouldHandleMessage(createPostMessageEvent("invalid"));
    ...
    it("should discard an invalid JSON data from right origin", async () => {
        psElevateTokenHandler.setVisualforceOriginURLs(mockDomainInfo());
        await flushPromises();
        const isMessageHandled = psElevateTokenHandler.shouldHandleMessage(createPostMessageEvent("invalidJSON"));
        expect(isMessageHandled).toBe(false);
    });
    ...
    it("should discard an invalid form JSON", async () => {
        psElevateTokenHandler.setVisualforceOriginURLs(mockDomainInfo());
        await flushPromises();
        const isMessageHandled = psElevateTokenHandler.shouldHandleMessage(createPostMessageEvent("stringifiedJSON"));
        expect(isMessageHandled).toBe(false);
    });
